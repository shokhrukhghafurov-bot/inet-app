    ...
    companion object {
        ...
        private fun fromMap(map: Map<*, *>): VpnConnectConfig? {
            ...
            val config = VpnConnectConfig(
                ...
                alpn = alpn,
                domainResolver = map["domainResolver"]?.toString(),
                packetEncoding = map["packetEncoding"]?.toString(),
                rawSingBoxConfig = map["rawSingBoxConfig"]?.toString(),
                rawXrayConfig = map["rawXrayConfig"]?.toString(),
            )
        ...
        private fun fromJsonObject(json: JSONObject): VpnConnectConfig? {
            ...
            val config = VpnConnectConfig(
                ...
                alpn = jsonArray("alpn"),
                domainResolver = json.optString("domainResolver", null),
                packetEncoding = json.optString("packetEncoding", null),
                rawSingBoxConfig = json.optString("rawSingBoxConfig", null),
                rawXrayConfig = json.optString("rawXrayConfig", null),
            )
            return config.takeIf { it.isComplete() }
        }
    }

...
class InetVpnService : VpnService() {
    ...
    private fun beginConnect(config: VpnConnectConfig) {
        ...
        stateStore.clearError()
        updateNotification("Connecting VLESS ${config.locationCode}")
        ...
        try {
            ...
            // and route packets from the TUN descriptor through the configured outbound.

            stateStore.setStatus(VpnStateStore.STATUS_CONNECTED)
            stateStore.setConnectedAt(System.currentTimeMillis())
            stateStore.clearError()
            updateNotification("Connected VLESS ${config.locationCode}")
    ...
    companion object {
        ...
        fun onAppBackgrounded(context: Context) {
            VpnStateStore(context).setPermissionRequired(false)
        }
    }
}

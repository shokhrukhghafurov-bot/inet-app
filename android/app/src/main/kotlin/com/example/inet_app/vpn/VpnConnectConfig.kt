    ...
    companion object {
        ...
        private fun fromMap(map: Map<*, *>): VpnConnectConfig? {
            ...
            val config = VpnConnectConfig(
                ...
                port = port,
                uuid = uuid,
                engine = map["engine"]?.toString() ?: "sing-box",
                ...
                rawSingBoxConfig = (map["rawSingBoxConfig"] ?: map["raw_sing_box_config"])?.toString(),
                rawXrayConfig = (map["rawXrayConfig"] ?: map["raw_xray_config"])?.toString(),
            )
        ...
        private fun fromJsonObject(json: JSONObject): VpnConnectConfig? {
            ...
            val config = VpnConnectConfig(
                ...
                engine = json.optString("engine", "sing-box"),
                ...
                rawSingBoxConfig = if (json.has("rawSingBoxConfig")) json.optString("rawSingBoxConfig", null) else json.optString("raw_sing_box_config", null),
                rawXrayConfig = if (json.has("rawXrayConfig")) json.optString("rawXrayConfig", null) else json.optString("raw_xray_config", null),
            )
            return config.takeIf { it.isComplete() }
        }
    }

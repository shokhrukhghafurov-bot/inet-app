...
/**
 Adapter between PacketTunnel and the real Libbox.xcframework / sing-box wrapper that you add to
 the Packet Tunnel target.

 How to finish the integration:
 1. link Libbox.xcframework or your own static wrapper into the PacketTunnel target
 2. add a factory class conforming to `IOSSingBoxRuntimeFactory`
 3. optionally set `SINGBOX_RUNTIME_FACTORY_CLASS` in the PacketTunnel Info.plist if the factory
    class name is not one of the defaults below

 After that, the bridge will manage lifecycle and tunnel shutdown for you.
 */
...
final class LibboxSingBoxBinding: NSObject, IOSSingBoxBinding {
  ...
  private static func initializeOptionalLibboxRuntime() throws {
    ...
    // Reserved for project-specific one-time libbox setup.
    // Most wrappers do not need anything here on iOS, but keeping the hook centralised avoids
    // duplicate init work across reconnects.
  }

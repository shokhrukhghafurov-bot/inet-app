...
final class IOSVpnBridge: NSObject {
  private let stateStore = IOSVpnStateStore()
  private let providerBundleIdentifier = "com.example.inetApp.PacketTunnel"
  ...
  func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "connect":
      guard
        ...
        let config = VlessConfigPayload(rawConfig),
        config.isComplete
      else {
  ...
  private func connect(config: VlessConfigPayload, result: @escaping FlutterResult) {
...
private struct VlessConfigPayload {
  init?(_ raw: [String: Any]) {
    self.raw = raw
    self.protocol = (raw["protocol"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false ? (raw["protocol"] as? String)! : "vless"
    self.locationCode = (raw["locationCode"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    self.server = (raw["server"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    self.port = raw["port"] as? Int ?? Int((raw["port"] as? String) ?? "") ?? 443
    self.uuid = (raw["uuid"] as? String)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    self.remark = raw["remark"] as? String
    self.transport = raw["transport"] as? String ?? "tcp"
    self.security = raw["security"] as? String ?? "reality"
    self.sni = raw["sni"] as? String
    self.host = raw["host"] as? String
    self.path = raw["path"] as? String
    self.serviceName = raw["serviceName"] as? String
    self.publicKey = raw["publicKey"] as? String
    self.shortId = raw["shortId"] as? String
    self.fingerprint = raw["fingerprint"] as? String
    self.allowInsecure = raw["allowInsecure"] as? Bool ?? false
    self.mtu = raw["mtu"] as? Int ?? Int((raw["mtu"] as? String) ?? "") ?? 1400
    self.dnsServers = raw["dnsServers"] as? [String] ?? ["1.1.1.1", "8.8.8.8"]
...
private final class IOSVpnStateStore {
  ...
  func setConfig(_ value: VlessConfigPayload?) {
    if let value {
      ...
      defaults.set(value.protocol, forKey: prefix + "protocol")
